...
// Typeahead for the Policies page — matches customer name OR policy number.
// Selecting a suggestion opens the policy; Enter/Search filters the list.
export default function PolicySearchBox({ initialValue = "" }: { initialValue?: string }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`/api/policies/suggest?q=${encodeURIComponent(q)}`);
  ...
  function onKeyDown(e: React.KeyboardEvent) {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setActive((a) => Math.min(a + 1, items.length - 1)); }
      ...
      e.preventDefault();
      if (active >= 0 && items[active]) openPolicy(items[active].id);
      else filterList();
    ...
    <div className="flex gap-2 mb-6">
      <div ref={boxRef} className="relative flex-1">
        ...
        {value && (
          <button
            type="button"
            onClick={() => { setValue(""); setItems([]); setOpen(false); router.push("/policies"); }}
            aria-label="Clear"
        ...
        {open && (
          <div className="absolute z-20 mt-1.5 w-full bg-white rounded-xl border border-slate-200 shadow-lg overflow-hidden">
            {loading && items.length === 0 ? (
              ...
              <p className="px-4 py-3 text-xs text-slate-400">No matching policies.</p>
            ) : (
              items.map((s, i) => (
                <button
                  key={s.id}
                ...
                >
                  ...
                  <span className="shrink-0 text-[10px] font-semibold uppercase tracking-wide text-indigo-600 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded-full">
                    Policy
                  </span>
                </button>
              ))
